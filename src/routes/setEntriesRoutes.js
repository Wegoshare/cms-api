...
const setEntriesRoutes = app => {
  ...
  app.get('/projects/:projectId/entries', cors(allowAll), checkAuth, checkProjectAccess, async (req, res) => {
    const { projectId } = req.params
    const entries = await apiEntries.getEntries(projectId)
    res.status(OK).send(entries)
  })

  app.post('/projects/:projectId/entries', cors(allowAll), checkAuth, checkProjectAccess, async (req, res) => {
    const { projectId } = req.params
